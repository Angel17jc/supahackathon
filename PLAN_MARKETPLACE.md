...
await supabase.from('products').select('cost_price')           // permission denied for table products
```
...
|---|---|---|
| 1 | Anónimo pide `cost_price` | `permission denied for table products`, mientras la misma consulta sin esa columna devuelve el catálogo entero |
